...
class PlutoniumFunctions {
    ...
    compress = {
        huffman: {
            ...
            generateCodes(root, codes = [], top = 0) {
                var res = {};
                if (root.left) {
                    codes.push(0);
            ...
            encode(clear) {
                ...
                for (var symbol in frequency) {
                    if (frequency.hasOwnProperty(symbol)) {
                        minHeap.push(new HuffmanNode(symbol.charCodeAt(0), frequency[symbol]));
                        n++;
                    }
                }
                var heapSize = n;
                ...
                while (heapSize > 1) {
                    ...
                    heapSize--;
                    var newNode = new HuffmanNode(-1, left.frequency + right.frequency);
                    newNode.left = left;
                    newNode.right = right;
