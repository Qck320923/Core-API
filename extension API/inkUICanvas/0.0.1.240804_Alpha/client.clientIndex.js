...
v0.0.1.240703_Alpha02 - 修改InkUICanvasEventsListenerParams为JsDoc定义类型和属性，添加listener的pressPixel和pressGraphic参数，并为参数添加JsDoc注解。
v0.0.1.240804_Alpha - 改正ColorCode中的darkdray为darkgray。添加searchGraphic和deleteGraphic方法，区域图形和像素搜索有待完善。
*/
/**
 * @typedef {object} InkUICanvasGraphicSearchOptions
 * @property {string} name 名字
 * @property {string} graphic 图形
 * @property {Bounds2} bounds 区域
 * @property {Vec2} offset 偏移量
 */
...
class InkUICanvas {
    ...
    #traversePixels(callback) {
        ...
        function traverse(node, parent) {
            ...
            for (let child of node.children) traverse(child, parent);
        };
        for (let child of this.#node.children) traverse(child, this);
    }
    /**
     * @description 创建像素点
    ...
    #createPixel(x, y, style, size = this.#pxSize, parent = this.#node) {
        ...
        pixel.events.on("pointerdown", () => {
            for (let listener of this.#events.mousedown) listener({
                ...
                pressPixel: pixel,
                pressGraphic: pixel.parent.graphic,
                pressGraphicName: pixel.parent.name
            });
        });
        pixel.events.on("pointerup", () => {
            for (let listener of this.#events.mouseup) listener({
                pressBounds: new Bounds2(
                    Vec2.create({
                        x: pixel.position.offset.x + pixel.parent.position.offset.x,
                        y: pixel.position.offset.y + pixel.parent.position.offset.y
     ...
     */
    fillRect(x, y, width, height, name) {
        let parent = UiBox.create();
        parent.parent = this.#node;
        parent.graphic = "rectangle";
        parent.name = (name && name.length) ? name : parent.graphic;
        parent.backgroundOpacity = 0;
        parent.position.offset.copy(Vec2.create({ x, y }));
        for (let i = 0; i < width; i += this.#pxSize.x)
            for (let j = 0; j < height; j += this.#pxSize.y)
     ...
     * @param {string} name 名字
     */
    strokeRect(x, y, width, height, name) {
        let parent = UiBox.create();
        parent.parent = this.#node;
        parent.graphic = "rectangle";
        parent.name = (name && name.length) ? name : parent.graphic;
        ...
        for (let i = -this.lineWidth / 2; i < width + this.lineWidth / 2; i += this.#pxSize.x)
            for (let j = -this.lineWidth / 2; j < this.lineWidth / 2; j += this.#pxSize.y)
                this.#createPixel(i, j, this.fillStyle, Vec2.create({
        ...
        for (let i = -this.lineWidth / 2; i < width + this.lineWidth / 2; i += this.#pxSize.x)
            for (let j = height - this.lineWidth / 2; j < height + this.lineWidth / 2; j += this.#pxSize.y)
                this.#createPixel(i, j, this.fillStyle, Vec2.create({
                    x: ((((width + this.lineWidth / 2) - i) >= this.#pxSize.x) ? this.#pxSize.x : (width + this.lineWidth / 2) - i),
                    y: ((((height + this.lineWidth / 2) - j) >= this.#pxSize.y) ? this.#pxSize.y : (height + this.lineWidth / 2) - j)
                }), parent);
        for (let i = -this.lineWidth / 2; i < this.lineWidth / 2; i += this.#pxSize.x)
            for (let j = this.lineWidth / 2; j < height - this.lineWidth / 2; j += this.#pxSize.y)
                this.#createPixel(i, j, this.fillStyle, Vec2.create({
                    x: (((this.lineWidth / 2 - i) >= this.#pxSize.x) ? this.#pxSize.x : this.lineWidth / 2 - i),
                    y: ((((height - this.lineWidth / 2) - j) >= this.#pxSize.y) ? this.#pxSize.y : (height - this.lineWidth / 2) - j)
                }), parent);
        for (let i = width - this.lineWidth / 2; i < width + this.lineWidth / 2; i += this.#pxSize.x)
            for (let j = this.lineWidth / 2; j < height - this.lineWidth / 2; j += this.#pxSize.y)
                this.#createPixel(i, j, this.fillStyle, Vec2.create({
                    x: ((((width + this.lineWidth / 2) - i) >= this.#pxSize.x) ? this.#pxSize.x : (width + this.lineWidth / 2) - i),
                    y: ((((height - this.lineWidth / 2) - j) >= this.#pxSize.y) ? this.#pxSize.y : (height - this.lineWidth / 2) - j)
    ...
    /**
     * @description 查找图形
     * @param {InkUICanvasGraphicSearchOptions} options 选项
     * @returns {UiBox[]}
     */
    searchGraphic(options) {
        let res = [];
        for (let child of this.#node.children) {
            if (options.name && options.name !== child.name) continue;
            if (options.graphic && options.graphic !== child.graphic) continue;
            if (options.offset && options.offset !== child.position.offset) continue;
            /* if (options.bounds && ) continue; */
            /* 包括对矩形、曲边图形的区域重合判断（曲边图形的区域不能用Bounds2表示。依次判断每个像素肯定可以，但是需要一些优化，因为像素可能过多） */
            res.push(child);
        }
        return res;
    }
    ...
    deleteGraphic(options) {
        for (let g of this.searchGraphic(options)) g.parent = undefined;
    }
}
